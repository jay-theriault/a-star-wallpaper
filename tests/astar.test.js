...
function parseKey(k){const [i,j]=k.split(',').map(Number); return {i,j};}
...
function makeGrid({cols, rows}){
  ...
  const cost = ()=>1;
  const heuristic = (a,g)=>{
    const A=parseKey(a), G=parseKey(g);
    return Math.abs(A.i-G.i)+Math.abs(A.j-G.j);
  };
  return {neighbors,cost,heuristic,isValidNode:(k)=>{const {i,j}=parseKey(k); return inBounds(i,j);}};
}
...
test('A* finds shortest path on empty grid (Manhattan)', ()=>{
  ...
  const stepper = makeAStarStepper({startKey:start, goalKey:goal, neighbors, cost, heuristic, isValidNode});
  const r = runToEnd(stepper);
  assert.equal(r.status,'found');
  // shortest length in nodes is 19 (18 moves)
  assert.equal(r.path.length, 19);
  // path adjacency
  for(let i=1;i<r.path.length;i++) assert.ok(isNeighbor(r.path[i-1], r.path[i], neighbors));
});
...
test('A* returns invalid-endpoints when endpoints fail validation', ()=>{
  ...
  const stepper = makeAStarStepper({startKey:'99,99', goalKey:gridKey(1,1), neighbors, cost, heuristic, isValidNode});
  const r = stepper.step();
...
test('A* stops with max-steps', ()=>{
  ...
  const stepper = makeAStarStepper({startKey:gridKey(0,0), goalKey:gridKey(49,49), neighbors, cost, heuristic, isValidNode, maxSteps:10});
